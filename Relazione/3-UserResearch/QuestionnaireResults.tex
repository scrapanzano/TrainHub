...
To empirically validate our design hypothesis, the Google Forms survey was distributed to a varied sample of individuals involved in the fitness ecosystem. The questionnaire collected a total of 81 valid responses. By utilizing conditional logic branches within the form, we ensured that both gym members and professionals could provide specific, highly relevant insights based on their distinct operational contexts.
...
\paragraph{Open-Ended Responses on Existing Applications} Furthermore, the questionnaire included an open-ended question specifically directed at gym members, asking: \textit{"If you use one or more applications, please write their names in the field below"}. This specific inquiry proved to be extremely valuable for our overall market research. It allowed us to deepen and empirically validate the findings discussed during the Competitor Analysis phase (Chapter \ref{sect:CompetitorsAnalysis}). By collecting the exact names of the tools currently adopted by our real target audience, we were able to cross-reference our initial data, confirm our list of direct and indirect competitors, and ultimately prove the strong fragmentation that characterizes the current digital fitness landscape.
...
The surveyed professionals manage varying client loads, from a single individual up to groups of more than 30 clients (Figure \ref{img:QuestionnaireClients}). Figure \ref{img:QuestionnaireMonitoring} shows a continued reliance on generic tools: spreadsheets and PDFs lead (71\%), closely followed by messaging apps and the gym's own internal software (57\% each). On management difficulties (Figure \ref{img:QuestionnaireChallenges}), a plurality reported no significant issues at this stage (43\%), though the sample is small; among those who did report a concrete pain point, the time required to create and update training and diet plans was the most common (29\%), ahead of communication dispersion and appointment management (14\% each). Even a partial signal here is consistent with the workflow TrainHub's professional dashboard is designed to consolidate.
